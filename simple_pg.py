...
def train(env_name='CartPole-v0', hidden_sizes=[32], lr=1e-2, 
    ...
    def train_one_epoch():
        ...
        step_rewards = []            # list for rewards accrued throughout ep
        ...
        while True:
            ...
            if done:
                ...
                total_reward, episode_length = sum(step_rewards), len(step_rewards)
                batch_returns.append(total_reward)
                batch_lengths.append(episode_length)

                # the weight for each logprob(a|s) is R(tau)
                batch_weights += [total_reward] * episode_length

                # reset episode-specific variables
                obs, done, step_rewards = env.reset(), False, []
